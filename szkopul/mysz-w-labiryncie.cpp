...
void dfs(pair<int, int> cur_pos){
    ...
    for(int i=0; i<moves.size(); i++){
        ...
        if (mod_pos.first >= n_rows || mod_pos.second >= n_cols) continue;

        if (mod_pos.first < 0 || mod_pos.second < 0) continue;

        if (maze[mod_pos.first][mod_pos.second] == 'x') continue;

        if (visited[mod_pos.first][mod_pos.second]) continue;
...
int main(){
    ...
    for(int i=0; i<n_rows; i++){
        ...
        for(int j=0; j<n_cols; j++){
            cin >> maze[i][j];
            if (maze[i][j] == 'w') exit_position = {i, j};
        }
    }
    dfs(exit_position);
    cout << "NIE";
